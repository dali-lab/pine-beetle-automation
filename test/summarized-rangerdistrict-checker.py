...
sourceOfTruthFile = sys.argv[1] # "./rangerdistrict-summarized-all.csv"
testFile = sys.argv[2] # "./no-2020:2021/outputs/rangerdistrict-summarized-with-missing-spots.csv"
...
mismatches = ["year,state,rangerDistrict,fieldName,sourceValue,testValue"]

for testRowIndex in testData.index:
    year = testData["year"][testRowIndex]
    state = testData["state"][testRowIndex]
    rangerDistrict = testData["rangerDistrict"][testRowIndex]

    sourceRow = sourceData[(sourceData["year"]==year) & (sourceData["state"]==state) & (sourceData["rangerDistrict"]==rangerDistrict)]

    if len(sourceRow.index) == 0:
        # print(f"MISSING ROW: {year} {state} {rangerDistrict:15} in source data")
        continue
    ...
    for key in keysToCompare:
        ...
        if sourceVal != testVal:
            mismatches.append(f"{year},{state},{rangerDistrict},{key},{sourceVal},{testVal}")
            # print(f"MISMATCH: {year} {state} {rangerDistrict:75} {key} = {sourceVal:4} vs {testVal:4}")
